...
Đợt cập nhật này tập trung vào nâng cấp toàn diện giao diện theo hướng **tối giản**. Không có thay đổi nào về logic nghiệp vụ hay kết nối backend.
...
- Khi mật khẩu đang ẩn → hiện icon `Eye` (mở). Khi mật khẩu hiển thị → hiện icon `EyeSlash` (gạch chéo).
